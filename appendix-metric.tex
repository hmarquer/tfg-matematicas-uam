...
\begin{defn}[Convergencia uniforme]\label{defn:convergencia-uniforme}\mbox{}\\
    Sean $f, f_n \colon X \to Y$ para $n\in \N$ con $\left(Y, d\right)$ un \exhyperref[defn:esp-metrico]{esp-metrico}{espacio métrico}, decimos que $\left(f_n\right)_{n\in\N}$ converge uniformemente a $f$ en $X$ (escrito $f_n \xrightrightarrows[n\to\infty]{X} f$)
    \[\begin{aligned}
        &\iff \forall \varepsilon > 0 : \exists n_0 \in \N : \forall n \geq n_0 : \forall x \in X : d\left(f_n(x), f(x)\right) < \varepsilon \\
        &\iff \exhyperref[defn:convergencia]{convergencia}{\lim_{n\to\infty}} \sup_{x\in X} d\left(f_n(x), f(x)\right) = 0.
    \end{aligned}
    \]
\end{defn}
...
\begin{defn}[Convergencia uniforme sobre compactos]\label{defn:convergencia-uniforme-compactos}\mbox{}\\
    Sean $f, f_n \colon X \to Y$ para $n\in\N$ con $\left(Y, d\right)$ un \exhyperref[defn:esp-metrico]{esp-metrico}{espacio métrico}, decimos que $\left(f_n\right)_{n\in\N}$ converge uniformemente sobre compactos a $f$ en $X$ (escrito $f_n \xRightarrow[n\to\infty]{X} f$)
    \[
...
\begin{prop}\label{prop:envoltura-convexa-combinacion-convexa}
    ...
    \[
    \operatorname{conv}(A) = \left\{v \in V : v \tex{ es una \exhyperref[defn:combinacion-convexa]{combinacion-convexa}{combinación convexa} de elementos de } A \right\}.
    \]
\end{prop}
