...
\begin{frame}[plain, noframenumbering]{Índice}
    ...
    \note[item]{
        Para esta presentación, he tenido que hacer una selección de los contenidos de la memoria que me han parecido más interesantes o relevantes, dejando fuera muchas cuestiones por falta de tiempo.
    }
    \note[item]{
        He organizado los contenidos de la presentación siguiendo la misma estructura que el TFG: cada sección de la presentación corresponde a un capítulo de la memoria.
    }
    \note[item]{
        En lugar de seguir una estructura estrictamente lineal a lo largo de todo el trabajo que culmine en un solo resultado final, cada capítulo representa un enfoque diferente de manera que se complementan entre sí.

        Dicho esto, no son completamente independientes entre sí, sino que cada capítulo se apoya en los resultados anteriores.
    }
    \note[item]{
        El primer capítulo sirve de introducción de los conceptos y resultados más básicos, el segundo, se centra en reinterpretar y generalizar bajo un enfoque geométrico el teorema de Schwarz-Pick y, en ese sentido, es el más independiente de los demás. En el tercer capítulo, nos centramos en los productos de Blaschke finitos y sus propiedades y en el cuarto capítulo, vemos qué usos tienen los productos de Blaschke finitos en la teoría de la aproximación.
    }
\end{frame}
}
...
\begin{frame}{La clase de Schur y el lema de Schwarz}
    ...
    \note<1-2>[item]{
        Como punto de partida, definimos la clase de Schur, que es el conjunto de funciones holomorfas en el disco unidad que toman valores en el disco cerrado.
    }
    \note<2>[item]{
        y consideramos el lema de Schwarz, un resultado fundamental en análisis complejo que asegura que una función de la clase de Schur que anule en cero no puede aumentar la distancia al origen:
        \begin{itemize}
            \item ni a nivel global, como muestra la primera desigualdad,
            \item ni localmente, como muestra la segunda desigualdad.
        \end{itemize}
    }
    \note<2>[item]{
...
\begin{frame}{Automorfismos del disco unidad}
    ...
    \note<1>[item]{
        Un subconjunto importante de la clase de Schur es el de automorfismos del disco unidad, que son las funciones holomorfas y biyectivas del disco unidad en sí mismo.
    }
    \note<1>[item]{
        Estos automorfismos serán los bloques de construcción de los productos de Blaschke, como veremos más adelante.
    }
    ...
    \note<3>[item]{
        ...
        \begin{itemize}
            ...
            La demostración de esta caracterización se basa en el lema de Schwarz y en el hecho de que cada involución del disco unidad es su propio inverso en el grupo de automorfismos, que es una propiedad que se usa continuamente a lo largo del trabajo.
        \end{itemize}
    }
\end{frame}
...
\begin{frame}{Teorema de Schwarz-Pick}
    ...
    \note<1>[item]{
        Con este nuevo lenguaje de automorfismos del disco unidad, podemos generalizar el lema de Schwarz al teorema de Schwarz-Pick.
    }
    \note<1>[item]{
        En este caso, no exigimos que la función se anule en el origen, sino que consideramos cualquier función de la clase de Schur y obtenemos dos desigualdades en correspondencia con las del lema de Schwarz:
        \begin{enumerate}
...
\begin{frame}{Métrica pseudohiperbólica}
    ...
    \note<3>[item]{
        La intuición detrás de esta propiedad es la siguiente: para la métrica euclídea, las sucesiones de Cauchy problemáticas son aquellas que se escapan a la frontera del disco unidad, que no pertenece al espacio.
        \begin{itemize}
            \item Según la métrica pseudohiperbólica, estas sucesiones dejan de ser de Cauchy porque la distancia pseudohiperbólica entre sus términos no tiende a cero, sino a $1$, que es su valor máximo.
    ...
    \note<4>[item]{
        En ese sentido, la métrica pseudohiperbólica ``aleja'' el borde del disco lo suficiente como para que las sucesiones que se acercan a él no sean de Cauchy, pero se mantiene acotada.
    }
    \note<4>[item]{
        Por otro lado, la métrica de Poincaré ``aleja'' el borde del disco infinítamente, por lo que no es acotada.
    }
\end{frame}
...
\begin{frame}{Discos pseudohiperbólicos}
    ...
    \note[item]{
        ...
        Aquí, $c$ es el centro euclídeo y $z$ es el centro pseudohiperbólico.
    }
    \note[item]{
        Además, en ambos casos, el radio pseudohiperbólico es el mismo, solo cambia el módulo del centro $z$ y podemos ver que el radio euclídeo es menor cuanto más cerca esté el centro pseudohiperbólico $z$ del borde del disco unidad, poniendo de manifiesto la propiedad de que la métrica pseudohiperbólica ``aleja'' el borde del disco unidad.
    }
\end{frame}
...
\begin{frame}{La métrica de Poincaré es Riemanniana}
    ...
    \note<1>[item]{
        La principal razón para introducir la métrica de Poincaré es que, a diferencia de la pseudohiperbólica, está sí es Riemanniana.
    }
    \note<1>[item]{
...
\begin{frame}{Productos de Blaschke finitos}
    ...
    \note<1>[item]{
        ...
        Recordando la caracterización de los automorfismos del disco unidad, podemos ver que no es más que un producto de $n$ automorfismos del disco unidad.
    }
    % \note<1>[item]{
...
\begin{frame}{Valencia constante de los productos de Blaschke finitos}
    ...
    \note<2>[item]{
        Además, también tenemos $n$-valencia en el exterior del disco unidad y en la frontera del disco unidad, donde las soluciones son simples.
    }
    \note<2>[item]{
        Esta propiedad explica por qué en esta gráfica, cada color se repite exactamente tres veces, que es el grado del producto de Blaschke que se ha representado. 
    }
\end{frame}
...
\begin{frame}{Aproximación de funciones en la clase de Schur}
    ...
    \note<2>[item]{
        En este caso, no veremos la demostración porque utiliza conceptos que no he mencionado en la presentación, pero el primer paso es demostrar este lema auxiliar que, básicamente, nos dice que podemos aproximar cualquier función de la clase de Schur alrededor del origen por productos de Blaschke finitos de grado proporcional a la calidad de la aproximación que queramos.
    }
\end{frame}
...
\begin{frame}{Aproximación por combinaciones convexas de productos de Blaschke finitos}
    ...
    \note<1>[item]{
        Si queremos aproximar uniformemente en el disco cerrado, necesitaremos un conjunto de funciones aproximadoras más grande que los productos finitos de Blaschke y no podremos aproximar todas las funciones de la clase de Schur, solo las que se extienden de forma continua al borde. 
    }
    \note<1>[item]{
        En concreto, tenemos este teorema de Fisher que nos asegura que podemos aproximar uniformemente en el disco cerrado cualquier función del álgebra del disco unidad que tome valores en el disco cerrado por combinaciones convexas de productos de Blaschke finitos.
    }
    \note<2-5>{
    ...
    \note<3-5>[item]{
        Además, como $f$ pertenece a la clase de Schur y este conjunto ($t\bar{\mathbb{D}}$) es un subconjunto compacto de $\mathbb{D}$, por el teorema de Carathéodory, podemos aproximar $f$ por un producto de Blaschke finito uniformemente en dicho subconjunto compacto.
    }
    \note<4-5>[item]{
...
\begin{frame}{Generalización y recíproco del teorema de Rouché}
    ...
    \note<1>[item]{
        Antes de ver el recíproco, tenemos esta generalización del teorema de Rouché.
    ...
    \note<1>[item]{
        En lo único en lo que difiere del teorema de Rouché es que en el lado derecho de la desigualdad tenemos la suma de los módulos de ambas funciones en lugar de solo el módulo de una de ellas, por lo que la hipótesis es más débil y, como el resultado es el mismo, el resultado es más general.
    }
    \note<1>[item]{
    ...
    \note<2>[item]{
        La implicación de derecha a izquierda se deduce de este teorema de Glicksberg, mientras que para la implicación de izquierda a derecha se necesita el teorema de Helson-Sarason de la diapositiva anterior.
    }
...
\begin{frame}[standout]
    ...
    \printbibliography %REVISAR: añadir separación entre referencias y quitar del indice de arriba
    ...
    \note[item]{
        Esto concluye la presentación. Muchas gracias por su atención.
    }
    \note[item]{
        Doy paso al turno de preguntas.
    }
\end{frame}
